...
#    apt-get install --yes nmap curl iputils-ping psutils net-tools bind9-host bind9utils telnet && \
    apt-get install --yes libssl-dev libcurl4-openssl-dev libc-ares-dev libssl-dev libcurl4-openssl-dev libxml2-dev libzip-dev liblz4-dev libzstd-dev && \
    apt-get clean && \
    apt-get autoremove --yes && \
    rm -rf /var/lib/{apt,dpkg,cache,log}/

RUN docker-php-ext-configure bcmath && docker-php-ext-configure pdo_mysql && docker-php-ext-configure calendar && docker-php-ext-configure zip && \
    docker-php-ext-install bcmath pdo_mysql calendar zip && \
